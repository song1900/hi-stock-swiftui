...
struct Log {
     ...
     */
    enum Level {
        case debug, info, network, error, custom(category: String)
    ...
    static private func log(
        ...
        switch level {
        case .debug, .custom: logger.debug("\(logMessage, privacy: .public)")
        case .info: logger.info("\(logMessage, privacy: .public)")
        case .network: logger.log("\(logMessage, privacy: .public)")
